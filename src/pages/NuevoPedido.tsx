  ...
  const guardarPedido = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: ord, error: errO } = await supabase.from('pedidos').insert([{
        ...
        detalles_tejido: pedido.descripcion + (pedido.observaciones ? ' | ' + pedido.observaciones : ''), 
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 max-w-2xl mx-auto min-h-[60vh]">
      ...
      {paso === 2 && (
        <form onSubmit={guardarPedido} className="space-y-6 animate-fadeIn">
          ...
          <select className="w-full p-3 border-2 border-gray-100 rounded-xl outline-none focus:border-rose-300 bg-white" value={pedido.fabricante} onChange={e => setPedido({...pedido, fabricante: e.target.value})} required>
